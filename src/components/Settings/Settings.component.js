...
export class Settings extends PureComponent {
  ...
  handleUnlock = () => {
    this.setState({ parentLockOpen: false, isUnlocked: true });
  };

  handleLockCancel = () => {
    this.setState({ parentLockOpen: false });
    // Navigate back
  ...
  render() {
    ...
    const { parentLockOpen, isUnlocked } = this.state;
    const isSettingsLocation = location.pathname === '/settings';

    // Show parent lock on first render
    if (!isUnlocked && isSettingsLocation) {
      return (
        <ParentLock
          open={parentLockOpen}
